...
def project_id(client, manager_headers, team_id):
    response = client.post('/api/v1/projects', headers=manager_headers, json={
        'name': 'Test Project',
...
def test_update_project_name(client, manager_headers, project_id):
    response = client.put(f'/api/v1/projects/{project_id}', headers=manager_headers, json={
...
def test_update_project_as_developer_forbidden(client, auth_headers, project_id):
    response = client.put(f'/api/v1/projects/{project_id}', headers=auth_headers, json={
        'name': 'Hacked'
    })
    assert response.status_code == 403
