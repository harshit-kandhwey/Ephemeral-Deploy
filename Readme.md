...
This document is a full technical walkthrough (17 sections, deliberately thorough). Most readers want one path through it, not all of it:

| You're here to... | Start with | Then |
| --- | --- | --- |
| **Evaluate this for a hiring decision** | [Notes for Interviewers](#notes-for-interviewers) — the *why* behind the choices | [§1 What This Project Demonstrates](#1-what-this-project-demonstrates), then `docs/design-decisions.md` for the deep "why" on any specific choice |
| **Run it yourself** | [§17 Local Development](#17-local-development) (docker-compose, zero AWS needed) | [§15 Getting Started](#15-getting-started--first-deployment) if you want a real AWS deploy |
| **Understand the architecture end-to-end** | [§2 How It All Fits Together](#2-how-it-all-fits-together) | Sections 3–14 in order |
| **Operate a live deployment** | [§16 Day-to-Day Operations](#16-day-to-day-operations) | [§10 Blue-Green Deployment](#10-blue-green-deployment-prod), [§12 Monitoring Stack](#12-monitoring-stack) |
| **Contribute code** | `CONTRIBUTING.md` | `docs/design-decisions.md` — check for an existing anchor before re-deciding something |
...
| `ecs`             | ECS cluster, 3 task definitions (api/worker/beat), 3 ECS services (ECS Exec enabled on api/worker), App Auto Scaling (CPU-based), CloudWatch log groups, optional ALB (commented out) |
| `rds`             | PostgreSQL `db.t3.micro`, subnet group spanning private DB subnets, parameter group                                                                         |
| `elasticache`     | Redis `cache.t3.micro` (single-node replication group), subnet group, in-transit encryption + AUTH token                                                    |
...
This creates the schema and seeds four demo users: `admin`, `manager`,
`developer1`, `developer2`. Passwords are **never printed** (that leaked them to
CloudWatch and tripped CodeQL). Where they come from:

- **Local docker-compose**: the `SEED_*_PASSWORD` env vars if set, otherwise the
  obvious dev-only defaults in `init_db.py` (`ChangeMe-Admin-2024!`,
  `ChangeMe-Manager-2024!`, `ChangeMe-Dev-2024!`).
- **Deployed environments (dev/staging/prod)**: strong per-environment passwords
  generated by Terraform (`random_password`) and injected from Secrets Manager.
  They live in their own secret (`nexusdeploy/<env>/seed-secrets`), separate from
  the DB master credentials in `init-secrets`, so this read never exposes
  `DB_MASTER_PASSWORD`. Read them with:

  ```bash
  aws secretsmanager get-secret-value \
    --secret-id nexusdeploy/<env>/seed-secrets \
    --query SecretString --output text | jq -r '.SEED_ADMIN_PASSWORD'
  ```

  Seeding a production-config database (staging/prod) **refuses** to fall back to
  the dev defaults — if the injected passwords are missing, `init_db` raises
  rather than seed known-weak credentials.

### Try the API

```bash
# Get a token (local docker-compose: default admin password below)
curl -X POST http://localhost:5000/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username": "admin", "password": "ChangeMe-Admin-2024!"}'
